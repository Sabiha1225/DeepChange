...
class DMINet(nn.Module):
    def __init__(self, num_classes=2, drop_rate=0.2, normal_init=True, pretrained=False, show_Feature_Maps=False):
        ...
        self.resnet.load_state_dict(torch.load('/content/gdrive/MyDrive/Courses/CS_5824_AML/Project1/DMINet/pretrained/resnet18-5c106cde.pth'))
    ...
    def forward(self, imgs1, imgs2, labels=None):
        ...
        if self.show_Feature_Maps:
            savepath=r'temp'
            draw_features((F.interpolate(c1, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c1_img1.png".format(savepath),8,8)
            draw_features((F.interpolate(c2, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/c2_img1.png".format(savepath),8,16)
            draw_features((F.interpolate(c3, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/c3_img1.png".format(savepath),16,16)  
            draw_features((F.interpolate(c1_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c1_img2.png".format(savepath),8,8)
            draw_features((F.interpolate(c2_img2, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/c2_img2.png".format(savepath),8,16)
            draw_features((F.interpolate(c3_img2, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/c3_img2.png".format(savepath),16,16)
            # You can show more.
