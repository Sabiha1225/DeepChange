...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.val_f1_dict = {}
        self.val_precision_dict = {}
        self.val_recall_dict = {}
    ...
    def _collect_epoch_states(self):
        print("Inside the collect Epoch")
        ...
        for k, v in scores.items():
            message += '%s: %.5f ' % (k, v)
            if(k == 'acc' and self.is_training):
              self.accuracy_dict[self.epoch_id] = v
            elif(k == 'mf1' and self.is_training):
              self.f1_dict[self.epoch_id] = v
            elif(k == 'mprecision' and self.is_training):
              self.precision_dict[self.epoch_id] = v
            elif(k == 'mrecall' and self.is_training):
              self.recall_dict[self.epoch_id] = v  
            elif(k == 'acc' and self.is_training == False):
              self.val_accuracy_dict[self.epoch_id] = v
            elif(k == 'mf1' and self.is_training == False):
              self.val_f1_dict[self.epoch_id] = v
            elif(k == 'mprecision' and self.is_training == False):
              self.val_precision_dict[self.epoch_id] = v
            elif(k == 'mrecall' and self.is_training == False):
              self.val_recall_dict[self.epoch_id] = v
               
        self.logger.write(message+'\n')
        self.logger.write('\n')

        self.epoch_f1_scores.append(self.epoch_acc)
        #print(f"Epoch {self.epoch_id}: f1 score = {self.epoch_acc}")
    ...
    def train_models(self):
        ...
        print('Inside trainer train model function')
        #self._load_checkpoint()

        print(f'Epoch to start: {self.epoch_to_start}')
        print(f'Max epoch: {self.max_num_epochs}')
        ...
        print(f"Accuracy: {self.accuracy_dict}")
        print(f"F1: {self.f1_dict}")
        print(f"Precision: {self.precision_dict}")
        print(f"Recall: {self.recall_dict}")

        print(f"Val Accuracy: {self.val_accuracy_dict}")
        print(f"Val F1: {self.val_f1_dict}")
        print(f"Val Precision: {self.val_precision_dict}")
        print(f"Val Recall: {self.val_recall_dict}")
